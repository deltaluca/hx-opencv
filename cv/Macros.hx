    ...
    static function transformRet(e:Expr, t:ComplexType):Null<Expr> {
        ...
        function tret(e:Expr, t:ComplexType) {
            ...
            case TPath({name:"Array",params:[TPType(_)]}):
                Context.error("@:CvProc doesn't know how to handle generating body of Array returned methods", e.pos);
                null;
            case TPath(_):
                macro $e{typeToIdent(t)}.cvt($e);
            default:
                Context.error("@:CvProc doesn't know hot to handle generating body with this return type", e.pos);
                null;
            };
        }
